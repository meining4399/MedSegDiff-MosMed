...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        # Region-loss hyperparameters (only used by LossType.BCE_DICE).
        # pos_weight ~ 1 / positive_fraction balances the ~0.5% lesion ratio.
        self.bce_pos_weight = bce_pos_weight
        self.bce_weight = bce_weight
        self.tversky_weight = tversky_weight
        self.tversky_alpha = tversky_alpha  # FP penalty weight
        self.tversky_beta = tversky_beta    # FN penalty weight (beta>alpha -> recall)
    ...
    def p_mean_variance(
        ...
        # V2 UNet (version='new') returns 2 channels even with learn_sigma=False.
        # Collapse to 1 channel to match x_t (1 channel) for eps->xstart conversion.
        # This mirrors the BCE_DICE training-loss branch. Single-channel models
        # (BRATS/ISIC) are unaffected (shape[1]==1, nothing changes).
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
        ...
        elif self.loss_type == LossType.BCE_DICE:
            # Region-aware loss for heavily imbalanced segmentation (e.g. MosMed
            # lesions occupy ~0.5% of pixels). Pure MSE on epsilon collapses to
            # predicting all-zero. We therefore convert the model's prediction
            # to a denoised mask estimate (pred_xstart) and supervise IT against
            # the ground-truth binary mask `res` with weighted BCE + Tversky
            # (a Dice generalisation that penalises false negatives harder).
            model_output, cal = model(x_t, self._scale_timesteps(t), **model_kwargs)
            if self.model_var_type in [
                ModelVarType.LEARNED,
                ModelVarType.LEARNED_RANGE,
            ...
            # The V2 UNet emits 2 channels for the mask prediction even with
            # learn_sigma=False. Collapse to a single foreground map so that it
            # matches the 1-channel target `res` / `res_t`. This must happen
            # BEFORE the eps->xstart conversion, which asserts shape equality
            # against res_t ([B,1,H,W]).
            if model_output.shape[1] > 1:
                model_output = model_output[:, :1]

            # Recover the predicted clean mask (x_start) from whichever
            # quantity the model is parameterised to output. For the default
            # EPSILON parameterisation this is the standard one-step DDIM
            # estimate; for START_X it is the raw output; for PREVIOUS_X it is
            # derived from the predicted posterior mean.
            if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            pred_logit = pred_xstart

            # Ground truth in {0,1}. Keep it as floats for the region losses.
            gt = res[:, :1].float() if res.shape[1] > 1 else res.float()
            # Clamp logits for numerical stability of sigmoid (used implicitly
            # inside binary_cross_entropy_with_logits).
            pred_logit = pred_logit.clamp(-10, 10)

            # --- Weighted BCE: penalise false negatives heavily to counter the
            # ~0.5% positive ratio. pos_weight ~ 1/positive_fraction.
            pos_weight = th.full(
                (1,), self.bce_pos_weight, device=pred_logit.device, dtype=pred_logit.dtype
            )
            bce = th.nn.functional.binary_cross_entropy_with_logits(
                pred_logit, gt, pos_weight=pos_weight, reduction="none"
            )
            bce = bce.flatten(1).mean(dim=1)  # [B]

            # --- Tversky loss: like Dice but with separate FP/FN weights.
            # FN weight (beta) > FP weight (alpha) emphasises recall, which
            # matters when lesions are tiny and easy to miss.
            pred_prob = th.sigmoid(pred_logit)
            alpha, beta = self.tversky_alpha, self.tversky_beta
            tp = (pred_prob * gt).sum(dim=tuple(range(1, pred_prob.dim())))
            fp = (pred_prob * (1 - gt)).sum(dim=tuple(range(1, pred_prob.dim())))
            fn = ((1 - pred_prob) * gt).sum(dim=tuple(range(1, pred_prob.dim())))
            tversky = (tp + 1e-5) / (tp + alpha * fp + beta * fn + 1e-5)
            tversky_loss = 1.0 - tversky  # [B]

            terms["loss_diff"] = self.tversky_weight * tversky_loss + self.bce_weight * bce
            terms["loss_cal"] = mean_flat((res - cal) ** 2)
            if "vb" in terms:
                terms["loss"] = terms["loss_diff"] + terms["vb"]
            else:
