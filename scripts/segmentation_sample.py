...
from guided_diffusion.mosmedloader import MosMedDataset
...
def main():
    ...
    if args.data_name == 'ISIC':
        ...
        args.in_ch = 5
    elif args.data_name == 'MOSMED':
        tran_list = [transforms.Resize((args.image_size,args.image_size)),]
        transform_test = transforms.Compose(tran_list)

        ds = MosMedDataset(args.data_dir, transform_test, test_flag=True)
        args.in_ch = 2  # 1 CT channel + 1 noise channel
    else:
        tran_list = [transforms.Resize((args.image_size,args.image_size)), transforms.ToTensor()]
        transform_test = transforms.Compose(tran_list)
    ...
    for _ in range(len(data)):
        if args.max_samples and sampled >= args.max_samples:
            logger.log(f"reached --max_samples={args.max_samples}, stopping.")
            break
        sampled += 1
        ...
        if args.data_name == 'ISIC':
            ...
            # virtual_path like "study_0255_slice10.nii" -> "0255_slice10"
            slice_ID=path[0].split("study_")[-1].split(".nii")[0]
        ...
        for i in range(args.num_ensemble):  #this is for the generation of an ensemble of 5 masks.
            ...
            if args.version == 'new':
                # Diffusion samples are in [-1, 1] (background=-1, foreground=+1).
                # Map to [0, 1] so staple()/save_image and downstream eval treat
                # background as 0 and foreground as 1. Without this, staple's
                # internal s*mvres squares the values and (-1)^2=1 turns the
                # whole image white.
                s_last = (sample[:, -1, :, :] + 1.0) / 2.0
                enslist.append(s_last)
            else:
                enslist.append(co)
        ...
        if len(enslist) == 1:
            # With a single ensemble member, staple's internal s*mvres squares
            # the values and compresses them (0.7 -> 0.49). Skip it and use
            # the single sample directly.
            ensres = enslist[0]
        else:
...
def create_argparser():
    defaults = dict(
        ...
        debug = False,
        max_samples = 0,        # 0 = sample all; >0 = stop after N slices (quick eval)
    )
